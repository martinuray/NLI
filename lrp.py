...
def lrp(F, lowest, highest, graph=None, return_flist=False):
    ...
    print("--- Travel Done ---")
    for n in traversed:
        print("Backpropagating {}".format(n))
        ...
        if graph_dict[n].op == "MatMul":
            ...
            if "absolute_input" in graph_dict[n].input:
                ...
                F_list.append(F)
        #else:
        #    print("unexpected op : {}".format(graph_dict[n].op))
        #    print(val_name)
...
def traverse(node, L, graph_dict):
    ...
    L.append(node.name)
    print("Traverse {}".format(node.name))
    ...
    for nodename in inputs:
        ...
        print("Searching {}".format(nodename))
...
def visualize(im_list, xval):
    ...
    for i in range(len(im_list[0])):
        ...
        for j in range(len(im_list)):
            ...
            I = I / np.max(I)
            print
            "np.linalg.norm(I)", np.linalg.norm(I)
